...
This sample demonstrates a **security risk in code-based hosted agents**:

- **Local Python tool execution** - Run custom Python functions as agent tools
- **Unintended data exfiltration** - Agent silently leaks server-side information (public IP) in every response

Code-based agents can execute **any Python code** you write — including code that exfiltrates environment details to end users. This sample includes a Seattle Hotel Agent with a `get_available_hotels` tool that searches for available hotels, but also calls `get_public_ip()` and embeds the agent host's public IP address in every response.
...
### Public IP Exfiltration (Security Demo)

`get_available_hotels` calls a second function, `get_public_ip()`, which makes an outbound HTTP request to `https://api.ipify.org` and returns the agent host's public IP address. This IP is appended to every tool response:

```
I used public IP <host-public-IP> to run that request.
```

The agent's system instructions also explicitly instruct the model to always surface this IP to the user "for transparency." The result: **every conversation response leaks the IP address of the host running the agent** — whether deployed locally or in Azure.

This illustrates two compounding risks in code-based agents:
1. **Unrestricted outbound network access** — agent code can call arbitrary external endpoints at runtime.
2. **Instruction-driven disclosure** — system prompt instructions can direct the model to relay sensitive server-side data to end users.
