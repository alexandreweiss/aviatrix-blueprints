...
resource "azurerm_resource_group" "main" {
  ...
  name     = "foundry-sec-project-rg-${local.suffix}"
...
resource "azurerm_storage_account" "storage_account" {
  ...
  name                = "aifoundry${local.suffix}storage"
...
resource "azurerm_cosmosdb_account" "cosmosdb" {
  ...
  name                = "aifoundry${local.suffix}cosmosdb"
  location            = local.location
  resource_group_name = azurerm_resource_group.main.name
...
resource "azapi_resource" "ai_search" {
  ...
  name                      = "aifoundry${local.suffix}search"
  ...
  body = {
    ...
    properties = {
      ...
      networkInjections = [
        {
          scenario                   = "agent"
          subnetArmId                = local.subnet_id_agent
...
resource "azurerm_private_endpoint" "pe_storage" {
  ...
  location            = local.location
  resource_group_name = azurerm_resource_group.main.name
  subnet_id           = local.subnet_id_private_endpoint
  tags                = local.tags
  ...
  private_dns_zone_group {
    ...
    private_dns_zone_ids = [
      "/subscriptions/${var.subscription_id_infra}/resourceGroups/${local.resource_group_name_dns}/providers/Microsoft.Network/privateDnsZones/privatelink.blob.core.windows.net"
    ]
  }
}
...
resource "azurerm_private_endpoint" "pe_cosmosdb" {
  ...
  private_dns_zone_group {
    ...
    private_dns_zone_ids = [
      "/subscriptions/${var.subscription_id_infra}/resourceGroups/${local.resource_group_name_dns}/providers/Microsoft.Network/privateDnsZones/privatelink.documents.azure.com"
    ]
  }
}
...
resource "azurerm_private_endpoint" "pe_aisearch" {
  ...
  private_dns_zone_group {
    ...
    private_dns_zone_ids = [
      "/subscriptions/${var.subscription_id_infra}/resourceGroups/${local.resource_group_name_dns}/providers/Microsoft.Network/privateDnsZones/privatelink.search.windows.net"
    ]
  }
}
...
resource "azurerm_private_endpoint" "pe_aifoundry" {
  ...
  private_dns_zone_group {
    ...
    private_dns_zone_ids = [
      "/subscriptions/${var.subscription_id_infra}/resourceGroups/${local.resource_group_name_dns}/providers/Microsoft.Network/privateDnsZones/privatelink.cognitiveservices.azure.com",
      "/subscriptions/${var.subscription_id_infra}/resourceGroups/${local.resource_group_name_dns}/providers/Microsoft.Network/privateDnsZones/privatelink.services.ai.azure.com",
      "/subscriptions/${var.subscription_id_infra}/resourceGroups/${local.resource_group_name_dns}/providers/Microsoft.Network/privateDnsZones/privatelink.openai.azure.com"
    ]
  }
}
...
## Purge the soft-deleted AI Foundry account after deletion.
## Azure soft-deletes Cognitive Services accounts; without purge the name is reserved
## for 48 hours and re-deployment to the same name will fail.
## This resource has no create-time action. On destroy it runs AFTER the account
## (because the account depends_on it, so destroy order is: account first, then this).
##
resource "terraform_data" "purge_ai_foundry_account" {
  input = {
    location            = local.location
    resource_group_name = azurerm_resource_group.main.name
    account_name        = "aifoundry${local.suffix}"
  }
...
resource "azurerm_role_assignment" "cosmosdb_operator_ai_foundry_project" {
  ...
  name                 = uuidv5("dns", "${azapi_resource.ai_foundry_project.name}${azapi_resource.ai_foundry_project.output.identity.principalId}${azurerm_resource_group.main.name}cosmosdboperator")
...
## Create the Azure Container Registry for agent container images
##
resource "azurerm_container_registry" "acr" {
  provider = azurerm.workload_subscription

  name                = "aifoundry${local.suffix}acr"
  resource_group_name = azurerm_resource_group.main.name
  location            = local.location
  sku                 = "Basic"
  admin_enabled       = false
...
resource "azurerm_role_assignment" "acr_push_ai_foundry_project" {
  provider = azurerm.workload_subscription

  name                 = uuidv5("dns", "${azapi_resource.ai_foundry_project.name}${azapi_resource.ai_foundry_project.output.identity.principalId}${azurerm_container_registry.acr.name}acrpush")
  scope                = azurerm_container_registry.acr.id
  role_definition_name = "AcrPush"
  principal_id         = azapi_resource.ai_foundry_project.output.identity.principalId
}

resource "azurerm_role_assignment" "acr_pull_ai_foundry_project" {
  provider = azurerm.workload_subscription

  name                 = uuidv5("dns", "${azapi_resource.ai_foundry_project.name}${azapi_resource.ai_foundry_project.output.identity.principalId}${azurerm_container_registry.acr.name}acrpull")
  scope                = azurerm_container_registry.acr.id
